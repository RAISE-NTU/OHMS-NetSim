...
def generate_launch_description():
    ...
    print(f'Set the world (default marsyard2020), e.g. ros2 launch ohms_netsim dual_robot_sim.launch.py world:=forest')
    ...
    sim_config_path = os.path.join(ohms_netsim_share_dir, 'config', 'multi_robot_2uav_1ugv_' + world + '_sim.yaml')
    ...
    spawn_robot_python_source = PythonLaunchDescriptionSource(
        os.path.join(ohms_netsim_share_dir, 'launch', 'spawn_robot.launch.py')
    )
    ...
    ros_gz_config = os.path.join(ohms_netsim_share_dir, 'config', 'multi_robot_2uav_1ugv_rgbd_lidar_ros_gz_bridge.yaml') # Change here for relevant sensor config
    ...
    launch_list = [spawn_robot_1, spawn_robot_2, spawn_robot_3, ros_gz_bridge]
    if teleop_joy_params['enable_teleop_joy']:
